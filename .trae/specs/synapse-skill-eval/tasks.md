...
- [ ] Task 1: 创建 SkillRegistry 模块
  ...
  - [ ] SubTask 1.4: 实现 `get_stats(skill_name)` 方法，返回元数据字典
  - [ ] SubTask 1.5: 使用 JSON 文件持久化（`data/skill_registry.json`）
  - [ ] SubTask 1.6: 运行 `pytest tests/test_skill_registry.py -v`

- [ ] Task 2: 创建 SkillEvaluator 模块
  - [ ] SubTask 2.1: 创建 `meta/skill_evaluator.py`，实现 `SkillEvaluator` 类
  - [ ] SubTask 2.2: 实现 `evaluate(skill_name)` 方法，计算成功率
  - [ ] SubTask 2.3: 实现 `find_low_quality_skills(threshold=0.5)` 方法
  - [ ] SubTask 2.4: 实现 `generate_improvement_report()` 方法
...
- [ ] Task 3: 修改 SkillRouter 记录执行结果
  ...
  - [ ] SubTask 3.2: 在技能执行成功后调用 `registry.record_execution(skill_name, success=True, ...)`
  - [ ] SubTask 3.3: 在技能执行失败后调用 `registry.record_execution(skill_name, success=False, error=...)`
  - [ ] SubTask 3.4: 运行 `pytest tests/test_skills_routing.py -v` 确保无回归

- [ ] Task 4: 修改 SkillCreator 集成评估闭环
  - [ ] SubTask 4.1: 在 `generate_skill` 前检查 Registry，如果已有相似技能则跳过生成
  - [ ] SubTask 4.2: 生成新技能后自动 `registry.register(skill_name, description)`
  - [ ] SubTask 4.3: 在 `SkillRouter` 中定期（或按需）调用 `SkillEvaluator.evaluate()`
